...
try {
    ...
    $stmt = $conn->prepare("UPDATE administrativo SET nombre=?, apellido=?, rfc=?, f_nacimiento=?, correo=?, telefono=?, area=?, estado=? WHERE id_administrativo=?");
    $stmt->bind_param("sssssssss", $nombre, $apellido, $rfc, $f_nacimiento, $correo, $telefono, $area, $estado, $id);
    $stmt->execute();

    // borrar vehiculos previos para este administrativo
    $stmtDel = $conn->prepare("DELETE FROM vehiculo WHERE id_administrativo = ?");
    $stmtDel->bind_param("s", $id);
    $stmtDel->execute();

    // insertar vehiculos recibidos
    ...
    $conn->commit();
    echo "✅ Administrativo actualizado correctamente.";
} catch (Exception $e) {
    $conn->rollback();
